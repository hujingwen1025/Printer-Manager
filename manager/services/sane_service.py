...
def resolve_scanner(scanner):
    ...
    raise RuntimeError("Scanner is not reachable through SANE")
...
def run_scan(job):
    ...
    while process.poll() is None:
        ...
        if time.monotonic() >= deadline:
            ...
            try:
                ...
                process.kill()
            raise RuntimeError("The scan exceeded the configured time limit")
        time.sleep(0.5)
    ...
    if process.returncode and not pages:
        raise RuntimeError(_friendly_error(stderr))
    if not pages:
...
def _friendly_error(stderr):
    value = stderr.lower()
    mappings = (("device busy", "Scanner is busy"), ("no documents", "The document feeder is empty"), ("cover", "The scanner cover is open"), ("i/o error", "Scanner is unreachable"))
    for needle, message in mappings:
        if needle in value:
            return message
    return "The scan failed; check the device and try again"
